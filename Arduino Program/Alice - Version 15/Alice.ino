...
String  PValiKey  = "";                   // For printing only
...
void setup()
{
    ...
    Serial.println("System initialised, waiting for Bob to be ready.");
        ...
        switch (myDecoder.value)
        {
            ...
            case 691139:                              // Alice informing Bob that her photosensor is ready.
                if (LightRdy)                         // Alice can only inform Bob to start once.
                {
                    ...
                    Serial.println("Bob has indicated ready, begin intensity calibration.");
                }
                break;
            ...
            case 691143:                              // Alice informing Bob that calibration is completed.
                Serial.println("-----------------------------");
                Serial.println("System and light intensity calibration has completed.");     // Printing
                Serial.println("Key in 's' or 'S' to begin quantum key generation.");
                Serial.println("-----------------------------");
            ...
            case 691154:                              // Bob request to send Alice message
                Serial.println("(Listening to Bob. Console closed)");       // 
            ...
            default:                                  // Any other data that is not filtered
                if (EncryKey.length() < 16 && !KeyBuilt)    // if building encryption key
                {
                    ...
                    else if (myDecoder.value != Bases)
                    {
                      Serial.println("Base mismatched.");
                    }
    ...
    if (LastTask != "" && TaskTime(msTask))                 // If there are still task to be performed and delay have been reached
    {
        ...
        {                                                   // Divide 16 bit encryption key to 8bit validation and 8bit encryption key
            for (int i = 0; i < 8; i++)
            {
                ...
                PValiKey += String(EncryKey.charAt(i));
            }
            String EncKey = "";
            for (int i = 8; i < 16; i++)
            {
